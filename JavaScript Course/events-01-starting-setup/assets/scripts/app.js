...
class ProjectList {
	...
	connectDroppable(){
		...
		prjList.addEventListener('dragleave', event => {
			if(event.relatedTarget.closest(`#${this.type}-projects ul`) !== prjList){
		...
		prjList.addEventListener('drop', event => {
			...
			document.getElementById(prjId).querySelector('button:last-of-type').dispatchEvent(new MouseEvent('click'));
			prjList.parentElement.classList.remove('droppable');
			event.preventDefault();
