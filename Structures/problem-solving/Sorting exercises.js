...
function sorting(arr, comparator) {
	...
	for(let i = 1; i < arr.length ; i++){
		//is the current element at the right spot
		if(comparator(arr[i], arr[i-1]) < 0){
			let position = i-1;
			while(position >= 0 && comparator(arr[position], arr[i]) > 0){
				position--;
			}
			arr = arr.slice(0, position + 1).concat(arr.slice(i, i+1).concat(arr.slice(position + 1, i).concat(arr.slice(i+1))));
		}
	}
